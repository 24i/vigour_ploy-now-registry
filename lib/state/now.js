...
function getDeployments (state) {
  ...
  return now.get('list', state.token, 'deployments.*')
    .on('data', deployment => {
...
function getLinks (deployment) {
  ...
  root.progress.set({
    [key]: {
      request: now.get(`deployments/${did}/links`, root.token, 'files.*')
        .on('data', file => {
          if (!file) { return }
...
function getPkg (deployment) {
  ...
  root.progress.set({[key]: {
    request: now.get(`deployments/${did}/files/${fid}`, root.token, false)
      .on('data', pkg => {
        if (!pkg || !pkg.version) {
          return
        }
