...
function runProgress (state) {
  state.emit('info', `process count ${state.progress.keys().length}`)
...
function calculateRegistry (state) {
  ...
  state.deployments.each((d) => {
    ...
    const key = [
      String(d.name.compute()).replace(/\./g, '-'),
      String(d.pkg.version.compute()).replace(/\./g, '-'),
      String(d.pkg.env.compute()).replace(/\./g, '-')
    ].join('@')
